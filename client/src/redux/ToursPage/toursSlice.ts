...
const toursSlice = createSlice({
  ...
  reducers: {
    filterTours(state, action) {
      state.tours = action.payload.toursDefault.filter(
        (tour: ITour) => {
          //проверка региона
          const isRegionTour = action.payload.values.region
            ? tour.region === action.payload.values.region
            : true;

          const isMinPriceTour =
            action.payload.values.priceRangeMin !== undefined 
              ? tour.price >= action.payload.values.priceRangeMin                 
              : true;

          const isMaxPriceTour =
            action.payload.values.priceRangeMax !== undefined
              ? tour.price <= action.payload.values.priceRangeMax
              : true;

          const isDayTour = action.payload.values.duration
            ? tour.length_days >= action.payload.values.duration[0] &&
              tour.length_days <= action.payload.values.duration[1]
            : true;

          return isRegionTour && isMinPriceTour && isMaxPriceTour && isDayTour;
        }
