...
const AdminTable: React.FC = () => {
  const [extendedTours, setExtendedTours] = useState<DataType[]>([]);
  ...
  const columns: TableColumnsType<DataType> = [
    ...
    },
    {
      ...
      key: "id",
      render: (id) => (
        <Link to={`http://localhost:5173/tour_edit/${id}`}>
          {" "}
          Редактировать <DoubleRightOutlined />
        </Link>
      ),
    },
  ];

  const data: DataType[] = extendedTours
    .sort((a, b) => a.id - b.id)
    .map((tour: DataType) => {
      return {
        ...tour,
        key: tour.id,
        rout: `/tour/${tour.id}`,
        owner: tour.User.full_name,
        email: String(tour.User.email),
        phone: String(tour.User.telephone),
      };
    });
